...
test_that("Omip58ScatterChannels names the three scatter channels", {
  expect_equal(
    unname(Omip58ScatterChannels(BuildFrame())),
    c("FSC-A", "FSC-H", "SSC-A")
  )
})

test_that("Omip58ScatterChannels stops when a scatter channel is absent", {
  frame <- BuildFrame()
  expect_error(
    Omip58ScatterChannels(frame[, c("FSC-A", "SSC-A", "V510-A")]),
    "missing the scatter channel"
  )
})

test_that("ReadCompensationState calls an identity matrix already compensated", {
  frame <- BuildFrame()
  ...
  expect_equal(state$state, "already compensated")
...
test_that("TransformOmip58 applies arcsinh to the channels named and no others", {
  values <- matrix(c(0, 150, 1500, 10, 20, 30), ncol = 2,
                   dimnames = list(NULL, c("V510-A", "FSC-A")))
  out <- TransformOmip58(values, "V510-A", cofactor = 150)
  expect_equal(out[, "V510-A"], asinh(c(0, 1, 10)))
  expect_equal(out[, "FSC-A"], c(10, 20, 30))
})

test_that("TransformOmip58 rejects a cofactor that is not positive", {
  values <- matrix(1, ncol = 1, dimnames = list(NULL, "V510-A"))
  expect_error(TransformOmip58(values, "V510-A", cofactor = 0),
               "cofactor must be positive")
})

test_that("TransformOmip58 rejects a channel the matrix does not carry", {
  values <- matrix(1, ncol = 1, dimnames = list(NULL, "V510-A"))
  expect_error(TransformOmip58(values, "U450-A"), "missing the channel")
})
