...
test_that("SelectByHighestMarker rejects a channel that is absent", {
  ...
  expect_error(SelectByHighestMarker(table, "Comp-PE-A"), "is not in the expression table")
})

test_that("SelectByHighestMarker rejects an impossible count", {
  table <- MakeMedianTable(c(10, 90))

  expect_error(SelectByHighestMarker(table, "Comp-BUV395-A", 0), "must be 1 or more")
  expect_error(SelectByHighestMarker(table, "Comp-BUV395-A", 5), "only 2 clusters")
})
